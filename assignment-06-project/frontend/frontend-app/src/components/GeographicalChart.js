...
const GeographicalChart = () => {
    ...
    const { countries, setUserSelectedCountries, userSelectedCountries } = React.useContext(AppContext);
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            svg.append("g")
                ...
                .on('click', d => {
                    const found = countries.find(element => element['code'] === d.target.id.split('-')[2]);
                    if (!found) return;
                    setUserSelectedCountries(oldArray => [...new Set([...oldArray, found])]);
                })
            setLoaded(true);
        })();
    }, [loaded, countries, userSelectedCountries, setUserSelectedCountries]);

...
(function () {
  ...
  async function init() {
      const data = await csv("./data/iris.csv");
      console.log(data);
      ...
      const x = linear().domain(extent(data, d => d.sepalwidth)).range([margin.left, width - margin.right]);
      const y = linear().domain(extent(data, d => d.sepallength)).range([margin.top, height - margin.bottom]);
      const color_scale = ordinal(Tableau10);
      ...
      data_g.selectAll("circle")
          ...
          .enter()
          .append("circle")
          .merge(data_g.selectAll("circle"))
              .attr("cx", d => x(d.sepalwidth))
              .attr("cy", d => y(d.sepallength))
              .attr("r", 3)
              .attr("fill", (d, i) => color_scale(data[i].class));
  }
